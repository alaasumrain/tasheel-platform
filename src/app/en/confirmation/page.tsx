...
export const metadata: Metadata = {
	title: 'Request Confirmation | Tasheel Platform',
	description: 'Review your Tasheel order details and track your request status in real time.',
};
...
export default async function Page({ searchParams }: PageProps) {
	...
	if (!orderNumber) {
		return <OrderConfirmation locale="en" orderFound={false} orderNumber={null} />;
	}
	...
	try {
		...
		if (!order) {
			return <OrderConfirmation locale="en" orderFound={false} orderNumber={orderNumber} />;
		}

		const serviceRecord = order.service_slug ? await getServiceBySlug(order.service_slug) : null;
		const service = serviceRecord
			? convertToLegacyFormat(serviceRecord, 'en')
			: null;
		...
		console.error('Failed to render confirmation page:', error);
		return <OrderConfirmation locale="en" orderFound={false} orderNumber={orderNumber} />;
	}
}
